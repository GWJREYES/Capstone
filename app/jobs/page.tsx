...
export default function JobsPage() {
  const [jobs, setJobs] = useState<any[]>(MOCK_JOBS as any[])
  const [selectedJob, setSelectedJob] = useState<any>(null)
  const [search, setSearch] = useState('')
  const [tradeFilter, setTradeFilter] = useState('all')
  const [showNewJob, setShowNewJob] = useState(false)
  const [newJob, setNewJob] = useState({
    job_number: `JOB-00${(jobs.length + 1).toString().padStart(2, '0')}`,
    customer_name: '',
    address: '',
    city: '',
    state: 'MA',
  ...
  const filtered = jobs.filter((j) => {
    ...
    const matchSearch =
      ...
      j.customer.city.toLowerCase().includes(search.toLowerCase())
  ...
  const handleCreateJob = () => {
    const job = {
      ...
      customer: {
        name: newJob.customer_name || 'New Customer',
      ...
      subcontractor: newJob.subcontractor ? { company: newJob.subcontractor } : null,
    ...
    setShowNewJob(false)
    setNewJob({
      job_number: `JOB-00${(jobs.length + 2).toString().padStart(2, '0')}`,
      customer_name: '',
      address: '',
      city: '',
      state: 'MA',
    ...
    <div className="min-h-screen bg-[#09090b] pt-14 lg:pt-0">
      ...
      <div className="px-6 pt-6 pb-4 border-b border-[#2a2a32]">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="font-nav text-sm text-[#606070] mt-0.5">{filtered.length} of {jobs.length} jobs shown</p>
          </div>
          <button
      ...
      <div className="px-6 py-3 border-b border-[#2a2a32] flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[180px] max-w-xs">
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#606070] hover:text-[#e8e8ee]">
              <X size={12} />
            </button>
          )}
        </div>
          ...
          <tbody>
            {filtered.map((job) => (
              <tr
                ...
                className="border-b border-[#2a2a32]/50 table-row-hover"
              >
                ...
                <td className="px-4 py-3.5">
                  ...
                  <p className="font-nav text-[11px] text-[#606070]">{job.customer.address}, {job.customer.city}, {job.customer.state}</p>
                </td>
                <td className="px-4 py-3.5">
                ...
                <td className="px-4 py-3.5 hidden md:table-cell">
                  <span className="font-body text-sm text-[#9090a0]">
                    {job.subcontractor?.company || <span className="text-[#606070] italic">Unassigned</span>}
                  </span>
                </td>
                <td className="px-4 py-3.5">
      ...
      {showNewJob && (
        <div className="fixed inset-0 z-50 flex items-center justify-center overlay-backdrop p-4">
          <div
            className="w-full max-w-lg bg-[#0f0f12] border border-[#2a2a32] rounded-lg overflow-y-auto max-h-[90vh] animate-slide-up"
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#2a2a32]">
              ...
              <button onClick={() => setShowNewJob(false)} className="text-[#606070] hover:text-[#e8e8ee]">
                <X size={18} />
              </button>
            </div>
            <div className="p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-nav text-[10px] tracking-wider uppercase text-[#606070] mb-1 block">Job #</label>
                  <input value={newJob.job_number} readOnly
                    className="w-full bg-[#0f0f12] border border-[#2a2a32] rounded px-3 py-2 font-mono text-sm text-[#606070]" />
                </div>
                <div>
              ...
              <div>
                ...
                <input type="text" value={newJob.customer_name}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2">
                  ...
                  <input type="text" value={newJob.address}
                ...
                <div>
                  ...
                  <select value={newJob.state} onChange={(e) => setNewJob({ ...newJob, state: e.target.value })}
                    className="w-full bg-[#0f0f12] border border-[#2a2a32] rounded px-3 py-2 font-nav text-sm text-[#e8e8ee] input-gold">
                    <option>MA</option><option>RI</option><option>CT</option><option>NH</option>
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={newJob.city}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={newJob.subcontractor}
                    onChange={(e) => setNewJob({ ...newJob, subcontractor: e.target.value })}
                    placeholder="Company name"
                    className="w-full bg-[#0f0f12] border border-[#2a2a32] rounded px-3 py-2 font-body text-sm text-[#e8e8ee] placeholder-[#606070] input-gold" />
                </div>
                <div>
                  <label className="font-nav text-[10px] tracking-wider uppercase text-[#606070] mb-1 block">Quoted Value ($)</label>
                  <input type="number" value={newJob.quoted_value}
                    onChange={(e) => setNewJob({ ...newJob, quoted_value: e.target.value })}
                    placeholder="0.00"
              ...
              <div>
                ...
                <input type="url" value={newJob.matterport_url}
                  onChange={(e) => setNewJob({ ...newJob, matterport_url: e.target.value })}
              ...
              <div>
                <label className="font-nav text-[10px] tracking-wider uppercase text-[#606070] mb-1 block">OneDrive / Video URL</label>
                <input type="url" value={newJob.onedrive_url}
                  onChange={(e) => setNewJob({ ...newJob, onedrive_url: e.target.value })}
                  placeholder="https://onedrive.com/..."
                  className="w-full bg-[#0f0f12] border border-[#2a2a32] rounded px-3 py-2 font-mono text-xs text-[#e8e8ee] placeholder-[#606070] input-gold" />
              </div>
              <div>
                <label className="font-nav text-[10px] tracking-wider uppercase text-[#606070] mb-1 block">Rilla Call URL</label>
                <input type="url" value={newJob.rilla_url}
                  onChange={(e) => setNewJob({ ...newJob, rilla_url: e.target.value })}
            ...
            <div className="px-5 pb-5 flex gap-3">
              <button onClick={handleCreateJob}
                className="flex-1 px-4 py-2.5 bg-[#c8922a] hover:bg-[#e8aa40] rounded-md font-nav text-sm font-semibold text-[#09090b] transition-colors">
                Create Job
              </button>
